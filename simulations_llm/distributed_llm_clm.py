...
class PrecomputedSampler(Sampler):
    def __init__(self, rank,  num_epochs, model_name_or_path=None):
        ...
        if "gemma" in model_name_or_path:
            self.indices_list_for_all = pickle.load(open("models/indices_gemma_{}.pkl".format(total_rank), "rb"))
        else:
            self.indices_list_for_all = pickle.load(open("models/indices_{}.pkl".format(total_rank), "rb"))
...
def evaluate(accelerator, model, eval_dataloader, args, completed_steps):
    model.eval()
    losses = []
...
def train(comm_state, sampler):
    ...
    eval_loss, perplexity = 1.7564435005187988, 5.791802180139986

    logger.info(f"step {completed_steps}: perplexity: {perplexity} eval_loss: {eval_loss} train_loss: NAN")

    if accelerator.is_main_process:
        txt_log_file.write(f"step {completed_steps}: perplexity: {perplexity} eval_loss: {eval_loss} train_loss: NAN\n")
        txt_log_file.flush()
    ...
    for epoch in range(starting_epoch, real_num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if completed_steps % 10 == 0:
                ...
                logger.info(f"step {completed_steps}: loss: {total_loss.item() / total_steps}")
                if accelerator.is_main_process:
                    txt_log_file.write(f"Step updates: step {completed_steps}: perplexity: / eval_loss: / train_loss: {total_loss.item() / total_steps} train_loss_per_10_steps: {total_losses_per_step[-1] / 10}\n")
                    txt_log_file.flush()
                total_losses_per_step.append(0)

            if isinstance(checkpointing_steps, int):
                if completed_steps % checkpointing_steps == 0 and completed_steps > 0:
                    eval_loss, perplexity = evaluate(accelerator, model, eval_dataloader, args, completed_steps / checkpointing_steps)
                    model.train()
                    logger.info(f"step {completed_steps}: perplexity: {perplexity} eval_loss: {eval_loss} train_loss: {total_loss.item() / total_steps}")

                    if accelerator.is_main_process:
                        txt_log_file.write(f"Epoch updates: step {completed_steps}: perplexity: {perplexity} eval_loss: {eval_loss} train_loss: {total_loss.item() / total_steps}\n")
                        txt_log_file.flush()
...
if __name__ == "__main__":
    ...
    if accelerator.is_main_process:
        if args.push_to_hub:
            ...
            api = HfApi()
            api.create_repo(repo_name, exist_ok=True, token=args.hub_token)
    ...
    raw_datasets["train"] = raw_datasets["train"].select(range(200000))
    raw_datasets["validation"] = raw_datasets["validation"].select(range(20000))
    ...
    max_data_length = 0
    for data in tokenized_datasets["validation"]:
    ...
    train_dataloader = DataLoader(
        train_dataset, sampler=sampler, collate_fn=default_data_collator, batch_size=args.per_device_train_batch_size
    )
    ...
    lr_scheduler = torch.optim.lr_scheduler.LinearLR(
        ...
        start_factor=1.,
        end_factor=1. / 8.,
        total_iters=max(1, args.max_train_steps),
    )

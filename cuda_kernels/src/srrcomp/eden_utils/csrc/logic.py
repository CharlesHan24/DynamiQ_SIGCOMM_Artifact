...
class Aee_Dynamic_Range(object):
    def __init__(self, nbits, chunk_size, params):
        ...
        self.randomized_vec_pool = torch.randint(0, 65536, dtype=torch.float32, size=(self.three_times_cur_max_size // chunk_size, chunk_size), device="cuda:0") / 65536
    ...
    def create_tensor(self, tensor, dtype):
        ...
        return torch.zeros(padded_vec_size, dtype=dtype, device="cuda:0")
...
class Aee_Dynamic_Range(object):
    def __init__(self, nbits, chunk_size, params):
        ...
        self.randomized_vec_pool = torch.rand(dtype=torch.float16, size=(self.three_times_cur_max_size, chunk_size), device="cuda:0") + 1 # NOTE: range is [1, 2]. Formula: floor(x) + ceil(prob - frac_x) = floor(x) + floor(prob + 1 - (x-floor(x)) = 2floor(x) + floor(prob + 1 - x)
        self.rand_seed = (self.nbits * 998244353) & 0xFFFFFFFF
    ...
    def dec_compression(self, recv_chunk, input_chunk, send_chunk, interm_chunk, interm_chunk2=None): # recv_chunk, input_chunk, send_chunk, interm_chunk are contiguous tensors. Decompress recv_chunk to interm_chunk, add interm_chunk to  input_chunk, compress input_chunk to send_chunk
        n = input_chunk.numel()
...
class Float16_compression(object):
    ...
    def create_tensor(self, tensor, dtype):
        ...
        return torch.zeros(n, dtype=torch.float16, device="cuda:0")
    
    def compression(self, input_chunk, send_chunk, interm_chunk): # send_chunk in float16; input_chunk in float32
        n = input_chunk.numel()
        send_chunk[:n].copy_(input_chunk)

    def decompression(self, recv_chunk, input_chunk, interm_chunk):
        n = input_chunk.numel()
        input_chunk.copy_(recv_chunk[:n])
    
    def dec_compression(self, recv_chunk, input_chunk, send_chunk, interm_chunk, interm_chunk2):
        n = input_chunk.numel()
...
class Float32_compression(object):
    ...
    def create_tensor(self, tensor, dtype):
        ...
        return torch.zeros(n, dtype=torch.float32, device="cuda:0")
    
    def compression(self, input_chunk, send_chunk, interm_chunk): # send_chunk in float32; input_chunk in float32
        n = input_chunk.numel()
        send_chunk[:n].copy_(input_chunk)
